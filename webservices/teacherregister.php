...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    $fquery = mysqli_query($db,"SELECT * FROM teacher WHERE email LIKE '%$data->email%'");
   ...
   /* echo json_encode($condition);
    ...
    if (strpos($data->email, $key) == true) {
        ...
        if($result > 0) { //check if there is already an entry for that username
            ...
            $query = mysqli_query($db, "INSERT INTO teacher (name, email, password, phone, code) VALUES ('$data->name','$data->email','$data->password','$data->phone', $code)");
            ...
            if($query){
             ...
             $json = array("status" => 0, "msg" => "The password should contain more than 6 characters");
        }
